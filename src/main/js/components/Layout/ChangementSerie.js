...
class ChangementSerie extends Component {
    ...
    renderConfirmModal = () => {
        ...
        if (this.state.placementsInfo && this.state.cuttingRequest && this.state.cuttingRequest.cuttingRequestPartNumbers) {
            ...
            this.state.cuttingRequest.cuttingRequestSeries.map((crs) => {
                ...
                arrDrillPlacement[crs.placement.trim().toUpperCase()] = arrDrill
            })
                ...
                if (this.state.placementsInfo[partNumber] != null) {
                    ...
                    sortedDigitKeys.map((digit, ind2) => {
                        ...
                        this.state.placementsInfo[partNumber][digit].placements.map(placement => {
                            const drillArr = arrDrillPlacement[placement.trim().toUpperCase()];
                            if (this.state.drillEmpArr[pattern.trim().toUpperCase()]) {
                                if (
                                    this.state.drillEmpArr[pattern.trim().toUpperCase()].drill1 > 0 && 
                                    (!drillArr || drillArr[0] === null || this.state.drillEmpArr[pattern.trim().toUpperCase()].drill1 + 1 < drillArr[0] || this.state.drillEmpArr[pattern.trim().toUpperCase()].drill1 - 1 > parseInt(drillArr[0]))
                                ) {
                                    erreurDrill1 = true;
                                }
                                if (
                                    this.state.drillEmpArr[pattern.trim().toUpperCase()].drill2 > 0 && 
                                    (!drillArr || drillArr[1] === null || this.state.drillEmpArr[pattern.trim().toUpperCase()].drill2 + 1 < drillArr[1] || this.state.drillEmpArr[pattern.trim().toUpperCase()].drill2 - 1 > parseInt(drillArr[1]))
                                ) {
                                    erreurDrill2 = true;
                                }
                            }
                            ...
                            <tr key={ind1 + "-" + ind2}>
                                ...
                                >
                                    {this.state.placementsInfo[partNumber][digit].placements && this.state.placementsInfo[partNumber][digit].placements.map(e => {
                                        if (arrDrillPlacement && arrDrillPlacement[e] &&  arrDrillPlacement[e.trim().toUpperCase()] && !(arrDrillPlacement[e][0] == null && arrDrillPlacement[e][1] == null)) {
    ...
    render() {
        return (
            <div>
                <h1 className='text-center' style={{ marginTop: 10, marginBottom: 20 }}>Vérification / changement des series coupe</h1>
...
export default connect(mapStateToProps, {})(ChangementSerie);