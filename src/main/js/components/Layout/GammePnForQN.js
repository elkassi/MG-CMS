...
class GammePnForQN extends Component {
			...
			.then(res => {
				...
				if (res.data && res.data.length > 0) {
					...
					if (this.state.type === "fabric") {
						...
						if (hasCNCPanel) {
							// Merge arr2 into arr1 for CNC gammes
							arr1 = [...arr1, ...arr2];
							
							// Extract CNC panel names (without "CNC" suffix) for display
							const cncPanelSet = new Set();
							arr1.forEach(elem => {
								if (elem.file.panelNumber && elem.file.panelNumber.toUpperCase().endsWith("CNC")) {
									const panelName = elem.file.panelNumber.substring(0, elem.file.panelNumber.length - 3).trim();
									cncPanelSet.add(panelName);
								}
							});
							cncPanels = Array.from(cncPanelSet);
						}
					}
				...
				let typesToInclude = this.state.type === "fabric" ? (hasCNCPanel ? ["fabric", "supplier kit fabric", "supplier kit leather", "cnc"] : ["fabric", "supplier kit fabric", "cnc"]) : [this.state.type];
					...
					.finally(() => {
						axios.get(`/api/gammeTechnique/${pn}`)
							.then(res => {
								...
								this.loadGammeTechniqueTexts(pn, finalArr).then(gammeTechniqueTexts => {
									this.setState({ arrPltFound, error: null, message: null, data: finalArr, arrPn: arrPn, pnInfo, pn, pndesc, item, itemcode5, leatherKit, supplierKit, ecnNumber, site, projet, heightRow: height, image, imageHeight, pnOther, cncPanels, gammeTechniqueTexts }, this.updatePnOrder)
								})
	...
	renderGamme = () => {
		...
		return <div style={{ position: "relative", paddingBottom: 15 }}>
			<div style={{
								...
								{arr.sort((a, b) => b.maxY - a.maxY).map((elem, ind) => {
									return (elem.pointsXY.length > 0 && <svg
										...
										onClick={() => {
											...
											if (this.props.updateSelectedIds) {
												if (this.state.selectedIds.includes(elem.file.panelNumber)) {
													this.setState({ selectedIds: this.props.updateSelectedIds(this.state.selectedIds.filter(e => e !== elem.file.panelNumber)) })
												} else {
													this.setState({ selectedIds: this.props.updateSelectedIds([...this.state.selectedIds, elem.file.panelNumber]) })
												}
											}
										}}
										//right click event
									...
									>
										{elem.pointsXY.map(points => <polyline
											...
											fill="none"
											stroke="black"
											style={this.state.selectedIds.includes(elem.file.panelNumber) ? { stroke: "blue", strokeWidth: 2 } : {}}
										/>)}
									</svg>)
								})}
							</div>
						...
						<div style={{ display: "flex" }}>
							...
							<div style={{ width: "34%" }}>
								...
								<div style={{ display: "flex", height: 24.13 }}>
									...
									<div className='gamme-header-elem' style={{ width: "60%" }}>{moment().format("YYYY-MM-DD,HH:mm")}</div>
...
export default connect(mapStateToProps, {})(GammePnForQN);
