import { faEye, faEyeSlash, faFloppyDisk, faFont, faLink, faPlus, faPrint, faTimes, faTrashCan } from '@fortawesome/free-solid-svg-icons';
...
import { buildGammeTextPayload, buildGammeTextResolveParams, createGammeTextDraft, getGammeTextsForPiece, normalizeGammeTexts } from './gammeTextUtils';
...
class GammePnPage extends Component {
	...
	nextTextClientId = () => {
		this.textCounter += 1;
		return `local-${Date.now()}-${this.textCounter}`;
	}
	...
	addGammeText = (piece, applyToPattern) => {
		if (!piece || !piece.file) {
			return;
		}
		if (applyToPattern && !piece.file.pattern) {
			this.setState({ error: "Cette pièce n'a pas de pattern pour créer un texte réutilisable." })
			return;
		}
		const text = createGammeTextDraft(piece, this.state.pn, applyToPattern, this.nextTextClientId())
		this.setState({
			gammeTechniqueTexts: [...this.state.gammeTechniqueTexts, text],
			selectedEmp: piece.file.panelNumber,
			selectedTextId: text.clientId,
			error: null,
			message: null
		})
	}

	updateSelectedGammeText = (changes) => {
		let selectedTextId = this.state.selectedTextId;
		if (!selectedTextId && this.state.selectedEmp) {
			const selectedObj = this.state.data.find(e => e.file != null && e.file.panelNumber == this.state.selectedEmp)
			const pieceTexts = this.getTextsForPiece(selectedObj)
			selectedTextId = pieceTexts.length > 0 ? pieceTexts[0].clientId : null
		}
		if (!selectedTextId) {
			return;
		}
		this.setState({
			gammeTechniqueTexts: this.state.gammeTechniqueTexts.map(text => {
				if (text.clientId !== selectedTextId) {
					return text;
				}
				return { ...text, ...changes }
			}),
	...
	removeSelectedGammeText = () => {
		...
		const text = this.state.gammeTechniqueTexts.find(elem => elem.clientId === selectedTextId)
		this.setState({
			gammeTechniqueTexts: this.state.gammeTechniqueTexts.filter(elem => elem.clientId !== selectedTextId),
			deletedGammeTechniqueTextIds: text && text.id ? [...this.state.deletedGammeTechniqueTextIds, text.id] : this.state.deletedGammeTechniqueTextIds,
			selectedTextId: null,
			error: null,
			message: null
		})
	...
	renderGammeTextElements = (arr, zoom, editable = false) => {
		let elements = []
		arr.forEach(elem => {
			this.getTextsForPiece(elem).forEach(text => {
				...
				const selected = this.state.selectedTextId === text.clientId
				elements.push(
					<text
						...
						onClick={editable ? (event) => {
							...
							this.setState({ selectedEmp: elem.file.panelNumber, selectedTextId: text.clientId })
						} : undefined}
					>
	...
	renderTextEditor = (selectedObj) => {
		if (!selectedObj || !selectedObj.file) {
			return <div className="gamme-text-editor gamme-text-editor-empty">
				<FontAwesomeIcon icon={faFont} /> Select a piece to add text.
			</div>
		}

		const pieceTexts = this.getTextsForPiece(selectedObj)
		const selectedText = pieceTexts.find(text => text.clientId === this.state.selectedTextId) || pieceTexts[0]

		return <div className="gamme-text-editor">
			<div className="gamme-text-editor-actions">
				<div className="gamme-text-piece">
					<strong>{selectedObj.file.panelNumber}</strong>
					<span>{selectedObj.file.pattern ? `Pattern ${selectedObj.file.pattern}` : "No pattern"}</span>
				</div>
				<button type="button" className="btn btn-sm btn-outline-primary" onClick={() => this.addGammeText(selectedObj, false)}>
					<FontAwesomeIcon icon={faPlus} /> PN text
				</button>
				<button type="button" className="btn btn-sm btn-outline-secondary" disabled={!selectedObj.file.pattern} onClick={() => this.addGammeText(selectedObj, true)}>
					<FontAwesomeIcon icon={faLink} /> Pattern text
				</button>
			</div>

			{selectedText && <div className="gamme-text-editor-grid">
				<select
					value={selectedText.clientId}
					onChange={event => this.setState({ selectedTextId: event.target.value })}
				>
					{pieceTexts.map(text => (
						<option key={text.clientId} value={text.clientId}>
							{text.applyToPattern ? "[Pattern] " : "[PN] "}{text.content}
						</option>
					))}
				</select>
				<input
					value={selectedText.content}
					onChange={event => this.updateSelectedGammeText({ content: event.target.value })}
					placeholder="Text"
				/>
				<label>
					X
					<input type="number" value={selectedText.labelX} onChange={event => this.updateSelectedGammeText({ labelX: event.target.value !== "" ? Number(event.target.value) : "" })} />
				</label>
				<label>
					Y
					<input type="number" value={selectedText.labelY} onChange={event => this.updateSelectedGammeText({ labelY: event.target.value !== "" ? Number(event.target.value) : "" })} />
				</label>
				<label>
					Size
					<input type="number" value={selectedText.labelSize} onChange={event => this.updateSelectedGammeText({ labelSize: event.target.value !== "" ? Number(event.target.value) : "" })} />
				</label>
				<select
					value={selectedText.fontWeight || "900"}
					onChange={event => this.updateSelectedGammeText({ fontWeight: event.target.value })}
				>
				...
				<select
					...
					onChange={event => this.updateSelectedGammeText({ fontStyle: event.target.value })}
				>
				...
				<input
					type="color"
					value={selectedText.fillColor || "#ff0000"}
					onChange={event => this.updateSelectedGammeText({ fillColor: event.target.value })}
					title="Text color"
				/>
				<label className="gamme-text-checkbox">
					<input
						type="checkbox"
						checked={selectedText.applyToPattern === true}
						disabled={!selectedObj.file.pattern}
						onChange={event => this.updateSelectedGammeText({
							applyToPattern: event.target.checked,
							pattern: selectedObj.file.pattern || selectedText.pattern,
							panelNumber: selectedObj.file.panelNumber,
							partNumberMaterial: selectedObj.file.partNumberMaterial
						})}
					/>
					Pattern
				</label>
				<button type="button" className="btn btn-sm btn-outline-danger" onClick={this.removeSelectedGammeText}>
	...
	saveGammeTechnique = () => {
		...
		axios.post("/api/gammeTechnique", {
			...
			.then(() => axios.post("/api/gammeTechniqueText/bulk", {
				partNumber: this.state.pn,
				texts: buildGammeTextPayload(this.state.gammeTechniqueTexts, this.state.pn),
				deletedIds: this.state.deletedGammeTechniqueTextIds
			}))
			.then(res => {
				this.setState({
					loading: false,
					message: this.state.pn + " is saved",
					gammeTechniqueTexts: normalizeGammeTexts(res.data || []),
					deletedGammeTechniqueTextIds: []
				})
			})
			.catch(err => {
				this.setState({ loading: false, error: err.response ? err.response.data : "Erreur sauvegarde gamme technique" })
			})
	}
	...
	searchPN = (pn, hr) => {
		...
		axios.get(`/api/cuttingPlan/ctc-info/${pn}/${this.state.type}`)
			.then(res => {
				...
				let finalArr = [], pnInfo = {}, height = this.state.heightRow

				axios.get(`/api/projet/search?nom=${projet}`)
					.then(res => {
						if (res.data != null && res.data.site) {
							site = res.data.site.nom
						}
					})
					.finally(() => {
							...
							.then(res => {
								if (res.data != null && res.data != "") {
									if (hr != null) {
										...
										height = res.data.heightRow ? res.data.heightRow : this.state.heightRow
									}
									// item = res.data.item ? res.data.item : this.state.item
									// itemcode5 = res.data.itemcode5 ? res.data.itemcode5 : this.state.itemcode5
									// leatherKit = res.data.leatherKit ? res.data.leatherKit : this.state.leatherKit
									// supplierKit = res.data.supplierKit ? res.data.supplierKit : this.state.supplierKit
									// ecnNumber = this.state.ecnNumber
									image = res.data.image ? res.data.image : this.state.image
									imageHeight = res.data.imageHeight ? res.data.imageHeight : this.state.imageHeight
								}

								arr.map(elem => {
									let oldEmpObj = (res.data != null && res.data != "") ? res.data.gammeTechniqueEmps.find(empObj => empObj.panelNumber === elem.file.panelNumber) : null
								...
								arrPn.map(pn => {
									...
									let oldPnInfo = (res.data != null && res.data != "") ? res.data.gammeTechniquePartNumberMaterials.find(pnObj => pnObj.partNumberMaterial.toUpperCase() === pn.toUpperCase()) : null
									...
									subarr.map(elem => {
										// x={parseInt((elem.x / zoom) + (elem.maxX / (zoom * 2)) - 8)} y={parseInt((elem.y / zoom) + elem.maxY / (zoom * 2))}
						...
						// Render the gamme immediately; loading the extra pattern texts must NOT gate the
						// gamme data. A slow/hanging /gammeTechniqueText/resolve previously left arrPn/data
						// unset, printing a blank page. Set data now; merge the texts when they arrive.
						this.setState({
							arrPltFound,
							error: null,
							message: null,
	...
	renderGamme = () => {
		...
		// Print: capture the in-flow A4-page wrapper (componentRef) and scale the rotated gamme to
		// fit it with MARGIN px all round. On screen (not rotated) keep the original hg x hg box.
		return <div ref={input => this.componentRef = input} style={rotated ? { position: "relative", width: PAGE_W, height: PAGE_H } : { position: "relative", height: hg, width: hg }} >
			<div style={rotated
				? {
						...
						return <div style={{
							...
							<div style={{
								...
								>
									...
									{this.renderGammeTextElements(arr, zoom, true)}
								</svg>
									...
									return (elem.pointsXY.length > 0 && <svg
										...
										onClick={() => {
											if (this.state.selectedEmp !== elem.file.panelNumber) {
												const pieceTexts = this.getTextsForPiece(elem)
												this.setState({ selectedEmp: elem.file.panelNumber, selectedTextId: pieceTexts.length > 0 ? pieceTexts[0].clientId : null })
											} else {
												this.setState({ selectedEmp: null, selectedTextId: null })
											}
										}}
										//right click event
										onContextMenu={(e) => {
		...
		return (
			<div className='' style={this.props.box ? {} : { padding: "20 50" }}>
				{this.props.box == null && <div className='d-flex'>
					...
					<button type="button" className="btn btn-success" onClick={this.saveGammeTechnique}><FontAwesomeIcon icon={faFloppyDisk} /></button>
					...
					<ReactToPrint ref={(component) => (this.printComponent = component)}
						onBeforeGetContent={() => {
							return new Promise((resolve, reject) => {
								this.setState({ modalRotate: true, selectedEmp: null, selectedTextId: null }, () => resolve())
							});
						}}
				...
				{this.props.box == null && this.renderTextEditor(selectedObj)}
				{this.renderGamme()}
				{this.renderConfirme()}
				{this.renderError()}
...
export default connect(mapStateToProps, {})(GammePnPage);
