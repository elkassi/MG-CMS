...
@RestController
@RequestMapping("/api/spliceMarkerLog")
public class MarkerLogController {
	
	@Autowired
	private MarkerLogService service;
	...
	private ResponseEntity<?> findAll(
		...
		filters.remove("size");
		filters.remove("sort");
		return new ResponseEntity<Page<MarkerLog>>(service.findAll(filters, page,size,sortBy), HttpStatus.OK);

	}

	@GetMapping("/serie/{serie}")
	public MarkerLog findBySerie(@PathVariable String serie) {

		List<MarkerLog> arr = service.findBySerie(serie);
		if(arr.size() ==0) return null;
		return arr.get(arr.size() - 1);
	}

	@GetMapping("/findTestsInShift")
	public List<MarkerLog> findTestsInShift(
		...
		return service.findTestsInShift(startDate, endDate);
